...
-- ----------------------------
INSERT INTO `users`
VALUES (1, '00000001', '123456', 'otto', 1, 24, '123456', '12345678910', 'cccccc', 123, '2023-10-16 01:20:59',
        '2023-10-16 01:21:02', '2023-10-16 01:21:05');

-- ----------------------------
-- View structure for goods
-- ----------------------------
DROP VIEW IF EXISTS `goods`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `goods` AS
select `t`.`value`       AS `type`,
       ...
       `p`.`type`        AS `type_id`,
       `p`.`sum`         AS `sum`
from (`power_types` `t` join `powers` `p`)
where (`t`.`id` = `p`.`type`);
